# modified Function's from Xiang's method

# IQKM (proposed method)
IQKM = function(Y, Z, K, Ada = FALSE){
  if(is.list(K)){
    nk = length(K)
    p.val.singleK = matrix(0, nrow = nk, ncol = 5)
    for(ik in 1:nk){
      results.temp = rep(0,5)
      results.temp[1] = fKMQR.test(Y,Z,tau = -1, score = "wilcoxon", K = K[[ik]])
      results.temp[2] = fKMQR.test(Y,Z,tau = -1, score = "normal", K = K[[ik]])
      results.temp[3] = fKMQR.test(Y,Z,tau = -1, score = "inverselehmann", K = K[[ik]])
      results.temp[4] = fKMQR.test(Y,Z,tau = -1, score = "lehmann", K = K[[ik]])
      if(Ada){
        Wt =c(0.5, 0.25, 0.25, 0)
       }else{
         Wt =c(0.25, 0.25, 0.25, 0.25)
         }
      results.temp[5] = pcauchy(sum(Wt*(tan((0.5-sort(results.temp[1:4]))*pi))), lower.tail = FALSE)
    ...
    results.temp = rep(0,5)
    results.temp[1] = fKMQR.test(Y,Z,tau = -1, score = "wilcoxon", K = K)
    results.temp[2] = fKMQR.test(Y,Z,tau = -1, score = "normal", K = K)
    results.temp[3] = fKMQR.test(Y,Z,tau = -1, score = "inverselehmann", K = K)
    results.temp[4] = fKMQR.test(Y,Z,tau = -1, score = "lehmann", K = K)
    if(Ada){
      Wt =c(0.5, 0.25, 0.25, 0)
    }else{
      Wt =c(0.25, 0.25, 0.25, 0.25)
